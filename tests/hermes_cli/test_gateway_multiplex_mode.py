...
def test_explicit_true_and_false_are_never_second_guessed(fleet, monkeypatch):
    ...
    # Explicit false remains an opt-out, regardless of whether the preflight would allow folding.
    monkeypatch.setenv("GATEWAY_MULTIPLEX_PROFILES", "false")
    pids.clear()
    cfg = load_gateway_config()
    decision = mode.resolve_multiplex_mode(cfg)
    assert decision == mode.MultiplexDecision(False, "config")
    assert cfg.multiplex_profiles is False
