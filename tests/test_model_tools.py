...
class TestHandleFunctionCall:
    ...
    def test_registry_exception_emits_terminal_tool_hook(self, monkeypatch):
        ...
        monkeypatch.setattr(
            lifecycle, "has_hook", lambda name: name == "post_tool_call"
        )
        monkeypatch.setattr(
            lifecycle,
            "invoke_hook",
...
class TestDisabledToolsetsPostureToolset:
    ...
    def test_disabling_coding_preserves_core_but_atomic_disables_still_remove(self):
        ...
        baseline = {
            t["function"]["name"] for t in get_tool_definitions(quiet_mode=True)
        }
        ...
        no_coding = {
            t["function"]["name"]
            for t in get_tool_definitions(disabled_toolsets=["coding"], quiet_mode=True)
        }
        ...
        no_file = {
            t["function"]["name"]
            for t in get_tool_definitions(disabled_toolsets=["file"], quiet_mode=True)
        }
