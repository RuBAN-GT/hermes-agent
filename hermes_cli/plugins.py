...
def _classify_entrypoint_value_kind(value: str) -> str:
    ...
    try:
        ...
        if not module_name:
            return "standalone"
        return (
            _detect_kind_from_source(_resolve_module_source(module_name))
            or "standalone"
        )
    except Exception:
        return "standalone"
...
def _parse_manifest_v2_fields(data: Mapping, key: str) -> Dict[str, Any]:
    ...
    if raw_api is None:
        ...
        try:
            ...
            logger.warning(
                "Plugin %s: api_version %r is not an integer; ignoring",
                key,
                raw_api,
            )
            out["api_version"] = None
    ...
    if raw_deps is not None and not isinstance(raw_deps, list):
        logger.warning(
            "Plugin %s: requires_plugins must be a list; ignoring",
            key,
        )
        raw_deps = None
    for item in raw_deps or []:
        if isinstance(item, str):
            ...
            isinstance(item, Mapping) and isinstance(item.get("id"), str) and item["id"]
        ):
    ...
    if raw_schema is not None and not isinstance(raw_schema, Mapping):
        logger.warning(
            "Plugin %s: config_schema must be a mapping; ignoring",
            key,
        )
        raw_schema = None
    for skey, spec in (raw_schema or {}).items():
        ...
        if stype is not None and str(stype).lower() not in _CONFIG_SCHEMA_TYPES:
            logger.warning(
                ...
                skey,
                stype,
                ", ".join(sorted(_CONFIG_SCHEMA_TYPES)),
            )
...
class PluginContext:
    ...
    def __init__(self, manifest: PluginManifest, manager: "PluginManager"):
        ...
        self._human_decisions: Any = None
    ...
    def human_decisions(self):
        """Return the capability-gated, session-bound decision facade.

        Decisions are currently rendered only by Telegram. The caller must
        provide an existing gateway session key; plugins never supply a chat id.
        """
        if self._human_decisions is None:
            from hermes_cli.plugin_human_decisions import PluginHumanDecisions

            self._human_decisions = PluginHumanDecisions(
                self.plugin_id,
                self._manager,
                lambda: self.has_capability("gateway.human_decisions"),
            )
            # Ownership-ledger cleanup makes disable/reload wake every waiter.
            self._track(
                "human_decisions", self.plugin_id, self._human_decisions.cancel_all
            )
    ...
    def has_capability(self, capability: str) -> bool:
        ...
        try:
            ...
            return False
        return plugin_capability_granted(plugin_id, capability, config=cfg)
    ...
    def register_command(
        ...
        The handler signature is ``fn(raw_args: str) -> str | None``. Handlers
        that accept a ``session_key`` keyword argument receive the current
        gateway session key when invoked from a gateway. It may also be an
    ...
    @_serialized_replacement
    def register_dashboard_auth_provider(
        self, provider
    ) -> Optional[PluginRegistration]:
    ...
    @_serialized_replacement
    def register_terminal_environment_provider(
        self, provider
    ) -> Optional[PluginRegistration]:
...
class PluginManager:
                ...
                if init_file.exists():
                    try:
                        detected = _detect_kind_from_source(
                            init_file.read_text(errors="replace", encoding="utf-8")[
                                :8192
                            ]
                        )
                        if detected:
    ...
    def _register_deferred_platform_tools(
        ...
        try:
            ...
            if not partial:
                ...
                scope = (
                    f"after registering {len(partial)} of {declared} declared tool(s)"
                )
            logger.warning(
                ...
                exc,
                ""
                if len(partial) >= declared
                else " The remainder will be missing from CLI/TUI sessions.",
                exc_info=_PLUGINS_DEBUG,
            )
    ...
    def _load_plugin_scoped(self, manifest: PluginManifest) -> None:
        ...
        try:
            ...
            if register_fn is None:
                ...
                _predeclared = [
                    t
                    for t in self._predeclared_tools.pop(plugin_key, [])
    ...
    def _reset_event_bus(self) -> None:
        ...
        with self._event_lock:
            ...
            self._event_pending_by_generation.setdefault(self._event_generation, 0)
    ...
    def get_portable_mcp_servers(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            name: dict(config) for name, config in self._portable_mcp_servers.items()
        }
...
def start_background_plugin_discovery() -> None:
    ...
    with _background_discovery_lock:
        if (
            _background_discovery_thread is not None
            and _background_discovery_thread.is_alive()
        ):
            return
    ...
    *,
    ...
    for result in hook_results:
        if not isinstance(result, dict):
            continue
        action = (
            str(result.get("action") or result.get("decision") or "").strip().lower()
        )
